    ...
    if (afs.exists(cRequire)) {
      ...
      parallel(
        ...
        function () {
          ...
          if (cli.argv['no-colors'] || cli.argv['color'] === false) {
            cmd.push('--no-colors');
          }
          ...
          function checkLine(line) {
            var re = new RegExp(
              '^(?:\u001b\\[\\d+m)?\\[?(' + logger.getLevels().join('|') + ')\\]?\s*(?:\u001b\\[\\d+m)?(.*)',
              'i'
            );
            if (line) {
              var m = line.match(re);
          ...
          if (process.platform === 'win32' && paths.alloy === 'alloy.cmd') {
            cmd.shift();
            logger.info(__('Executing Alloy compile: %s', ['cmd', '/s', '/c'].concat(cmd).join(' ').cyan));
          ...
          child.on('exit', function (code) {
            if (code) {
              ...
              afs.exists(path.join(cli.argv['project-dir'], 'build', 'i18n')) &&
                process.argv.push('--i18n-dir', 'build');
              afs.exists(path.join(cli.argv['project-dir'], 'build', 'platform')) &&
                (cli.argv['platform-dir'] = 'build/platform');
            }
            finished();
          });
        }
